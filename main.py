...
def col_selection():
    ...
    data = df[['Provider State', 'Provider Zip Code',
...
def data_cleaning(data):
    ...
    print(f"There are {num_of_nan} rows that have NaN"
          " in the 'Overall Rating' column")
    data = data.dropna()

    # clean ownership types
    # print(data['Ownership Type'].unique())
    data["Ownership"
         " adjusted"] = data["Ownership Type"
...
def health_vs_rating(data):
    ...
    data = data.merge(abbreviations, left_on="Provider State",
    ...
    data.plot(column="Total Weighted Health Survey Score",
              ax=ax2, legend=True, cmap='OrRd')
...
def health_geo_graphs(cycle1, cycle2, cycle3):
    ...
    cycle3 = cycle3[~test]
    cycle1["Rating Cycle 1"
           " Total Health Score"] = (cycle1["Rating Cycle 1 Total Health"
                                            " Score"].astype(float))
    cycle2["Rating Cycle 2"
           " Total Health Score"] = (cycle2["Rating Cycle 2 Total Health"
                                            " Score"].astype(float))
    cycle3["Rating Cycle 3"
           " Total Health Score"] = (cycle3["Rating Cycle 3 Total Health"
                                            " Score"].astype(float))

    cycle1 = cycle1.groupby("Provider State",
                            as_index=False)["Rating Cycle 1"
                                            " Total Health Score"].mean()
    cycle2 = cycle2.groupby("Provider State",
                            as_index=False)["Rating Cycle 2"
                                            " Total Health Score"].mean()
    cycle3 = cycle3.groupby("Provider State",
                            as_index=False)["Rating Cycle 3"
                                            " Total Health Score"].mean()
    ...
    cycle1 = cycle1.merge(abbreviations,
                          left_on="Provider State",
                          right_on="Code", how="left")
    cycle2 = cycle2.merge(abbreviations,
                          left_on="Provider State",
                          right_on="Code", how="left")
    cycle3 = cycle3.merge(abbreviations,
                          left_on="Provider State",
                          right_on="Code", how="left")

    cycle1 = country.merge(cycle1,
                           left_on="NAME", right_on="State", how="left")
    cycle2 = country.merge(cycle2, left_on="NAME",
                           right_on="State", how="left")
    cycle3 = country.merge(cycle3, left_on="NAME",
                           right_on="State", how="left")
    ...
    cycle1.plot(column="Rating Cycle 1 Total Health Score",
                ax=ax1, cmap='OrRd')
    cycle2.plot(column="Rating Cycle 2 Total Health Score",
                ax=ax2, cmap='OrRd')
    cycle3.plot(column="Rating Cycle 3 Total Health Score",
                ax=ax3, legend=True, cmap='OrRd')
...
def capacity_analysis(data):
    ...
    rnew["Percentage Filled"] = (rnew["Average Number of Residents per Day"] /
                                 new['Number of Certified Beds'])
...
def plot_staffing_hours(info):
    ...
    sns.kdeplot(
        data=info, x="Reported Total Nurse Staffing"
    ...
    ax[0].title.set_text("Average Residents Per Day vs"
                         " Nurse Staffing Hours Per Day")
...
def staffing_hours_data(data):
    """
    This function makes a graph that
    clearly shows that from the staffing
    hours fo the nursing homes. It makes
    graphs and displays them in
    comparison to the residents per day.
    ...
    mask1 = new['Reported Total Nurse Staffing '
                'Hours per Resident per Day'].isna()
    mask2 = new["Average Number of Residents per Day"].isna()
    mask3 = new["Overall Rating"].isna()
...
def provider_vs_rating(data):
    ...
    grouped = data.groupby("Ownership adjusted",
                           as_index=False)["Overall Rating"].mean()
    sns.barplot(x='Ownership adjusted',
                y='Overall Rating', data=grouped, ax=ax2)

    box_data = data
    ax = sns.boxplot(x="Ownership adjusted",
                     y="Overall Rating",
                     data=box_data, ax=ax1)
    ax.set(xticklabels=[])
    ax.set(xlabel=None)
...
def provider_vs_rating_geo(data_org):
    ...
    data = data_org[["Provider State", "Ownership adjusted", "Overall Rating",
    ...
    print(profit['Reported Total Nurse Staffing'
                 ' Hours per Resident per Day'].mean())
    print(nprofit['Reported Total Nurse Staffing'
                  ' Hours per Resident per Day'].mean())
    print(govt['Reported Total Nurse Staffing'
               ' Hours per Resident per Day'].mean())
